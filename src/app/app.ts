...
import { MetadataKeys, RouteDefinition, ITYPES, CTYPES, RTYPES } from "../utils/types";
...
myContainer.bind<IUserRepository>(RTYPES.IUserRepository).to(UserRepository);
...
class App {
  ...
  constructor(httPort: number, apiControllers: any, publicControllers: any = []) {
  ...
  private routerFactory(path: string, controllers: any) {
    ...
    controllers.forEach((ctrl: any) => {
      ...
      const ctrlInstance: { [handleName: string]: Handler } = myContainer.get(ctrl.name);

      const basePath: string = Reflect.getMetadata(MetadataKeys.BASE_PATH, ctrl);
      const routes: RouteDefinition[] = Reflect.getMetadata(MetadataKeys.ROUTES, ctrl);

      const router = express.Router();

      routes.forEach(({ method, path, handlerName }) => {
        router[method](path, ctrlInstance[String(handlerName)].bind(ctrlInstance));
